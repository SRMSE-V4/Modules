...
def gettitle(d,teams):
    import MySQLdb
    import urllib2
    db=MySQLdb.connect('127.0.0.1','root','root','rig')
    cursor=db.cursor()
    ...
    if '(' in str(content):
        ...
        if check:
            ...
            if (score1[0].split()[0] in check[0][4]) and len(bat)==3:
                cursor.execute("UPDATE cricket SET score='%s',teams='%s',overs='%s',bat1='%s',bat2='%s',bowl='%s',status='%s',description='%s' WHERE matchno='%s'"%(score1[0].strip(),teams,bat[0],bat[1],bat[2],bowl,status,str(num).replace('.html',''),desc)) 
                print "updated"
            elif (score1[0].split()[0] not in check[0][4]) and len(bat)==3:
                cursor.execute("UPDATE cricket SET score='%s',score1='%s',teams='%s',overs='%s',bat1='%s',bat2='%s',bowl='%s',status='%s',description='%s' WHERE matchno='%s'"%(score1[0].strip(),check[0][4],teams,bat[0],bat[1],bat[2],bowl,status,str(num).replace('.html',''),desc))
                print "updated1"
            elif (score1[0].split()[0] in check[0][4]) and len(bat)==2:
                cursor.execute("UPDATE cricket SET score='%s',teams='%s',overs='%s',bat1='%s',bowl='%s',status='%s',description='%s' WHERE matchno='%s'"%(score1[0].strip(),teams,bat[0],bat[1],bowl,status,str(num).replace('.html',''),desc)) 
                print "updated2"
